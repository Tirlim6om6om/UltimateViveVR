...
{
    /// <summary>
    /// Четсно взят отсюда: https://assetstore.unity.com/packages/tools/animation/unity-custom-ik-89426
    /// И немного доработан
    /// </summary>
    [ExecuteInEditMode]
    public class CustomIK : MonoBehaviour
    {
        ...
        {
            if (rootBone && elbowBone && endBone && targetIK && elbowIK)
                CalculateIK();
        }
        ...
        private void OnDrawGizmos()
        {
            if (targetIK)
            {
                Gizmos.color = new Color(0.2f, 0.7f, 0.9f);
                Gizmos.DrawWireSphere(targetIK.position, 0.1f);
            }

            if (elbowIK)
            {
                Gizmos.color = new Color(0.9f, 0.7f, 0.0f);
                Gizmos.DrawWireSphere(elbowIK.position, 0.1f);
            }
        }
        ...
        void CalculateIK()
        {
            ...
            float angleUpperArm =
                Vector3.Angle(jointPosition - rootBone.position, targetIK.position - rootBone.position);
            /*float targetAngleV;
        ...
        Vector3 CalculateElbowAdjacent()
        {
            ...
            float triangleHeight =
                CalculateTriangleHeight(upperArmToElbowLength, elbowToTargetLength, upperArmToTargetLength);
