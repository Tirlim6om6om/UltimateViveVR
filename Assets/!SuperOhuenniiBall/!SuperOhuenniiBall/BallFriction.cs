...
public class BallFriction : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        /*// Доступ к точке столкновения
        foreach (ContactPoint contact in collision.contacts)
        {
            Debug.DrawRay(contact.point, contact.normal, Color.white, 2.0f);
            Debug.Log("Точка столкновения: " + contact.point);
        }

        // Доступ к скорости столкновения
        Vector3 collisionVelocity = collision.relativeVelocity;
        Debug.Log("Скорость столкновения: " + collisionVelocity.magnitude);*/
    }
