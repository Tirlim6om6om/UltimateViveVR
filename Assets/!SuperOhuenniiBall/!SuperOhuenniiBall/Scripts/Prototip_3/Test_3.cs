...
public class Test_3 : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Ball"))
        {
            ...
            /*// Вычисляем среднюю скорость за последние несколько кадров, используемую как прокси силы удара
            float averageSpeed = totalRayLength / framesToTrack / Time.deltaTime * 10f; // Делим на framesToTrack и Time.deltaTime для получения средней скорости

            TextUI.text = averageSpeed.ToString("F2");
            
            // Обновляем impactForceCoefficient в соответствии с силой удара
            if (averageSpeed <= slowSpeedThreshold)
            {
                impactForceCoefficient *= weakUdar;
            }
            else if (averageSpeed > slowSpeedThreshold && averageSpeed <= highSpeedThreshold)
            {
                impactForceCoefficient *= mediumUdar;
            }
            else if (averageSpeed > highSpeedThreshold)
            {
                impactForceCoefficient *= strongUdar;
            }  */  
